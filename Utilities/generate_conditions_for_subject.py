...
def generate_trials(reference_color_lhs, comparison_colors_lhs, background_colors_lhs, fixed_background_lhs, num_trials, output_file="colors.txt", condition_file="condition.txt", base_folder="."):
    """
    ...
    Parameters:
        ...
        num_trials (int): Number of times each color combination should be repeated.
...
def generate_conditions_for_subject(subject_name, base_folder, num_repetitions):
    ...
    conditions = {
        "condition_1": {
            "reference_color_lhs": generate_lhs_tuples(0.30, 0.30, 0.01),
            "comparison_colors_lhs": generate_lhs_tuples(0.25, 0.355, 0.01),
            "background_colors_lhs": generate_lhs_tuples(0.25, 0.36, 0.05),
            "fixed_background_lhs": generate_lhs_tuples(0.3, 0.3, 0.01),
            "num_trials": 30,
        },
        "condition_2": {
            "reference_color_lhs": generate_lhs_tuples(0.50, 0.50, 0.01),
            "comparison_colors_lhs": generate_lhs_tuples(0.45, 0.555, 0.01),
            "background_colors_lhs": generate_lhs_tuples(0.45, 0.56, 0.05),
            "fixed_background_lhs": generate_lhs_tuples(0.5, 0.5, 0.01),
            "num_trials": 30,
        },
        "condition_3": {
            "reference_color_lhs": generate_lhs_tuples(0.70, 0.70, 0.01),
            "comparison_colors_lhs": generate_lhs_tuples(0.65, 0.755, 0.01),
            "background_colors_lhs": generate_lhs_tuples(0.65, 0.76, 0.05),
            "fixed_background_lhs": generate_lhs_tuples(0.7, 0.7, 0.01),
            "num_trials": 30,
        },
    }
    ...
    for condition_name, params in conditions.items():
        for repetition in range(1, num_repetitions + 1):
            ...
            generate_trials(
                reference_color_lhs=params["reference_color_lhs"],
                comparison_colors_lhs=params["comparison_colors_lhs"],
                background_colors_lhs=params["background_colors_lhs"],
                fixed_background_lhs=params["fixed_background_lhs"],
